...
class ModelArgs:
    ...
    num_layers = 12
    d_ff = 4 * 256
    conv_kernel = 7      # local convolution kernel size (this is the only model change vs super_runner)
...
# MODEL  (RoPE + pre-norm + SDPA + local convolution)
...
class LocalConv(nn.Module):
    # depthwise separable conv over the sequence — captures short-range motifs
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, padding_mask=None):
        ...
        if padding_mask is not None:
            h = h * padding_mask.unsqueeze(-1).to(h.dtype)   # zero pad positions so they don't bleed through the conv
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.ModuleList([EncoderLayer(ModelArgs.d_model, ModelArgs.num_heads,
                                                  ModelArgs.d_ff, ModelArgs.dropout, ModelArgs.conv_kernel)
